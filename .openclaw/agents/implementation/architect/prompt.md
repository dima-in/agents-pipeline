...
- Derived metrics must be JOIN-GROUNDED. Before planning any metric that combines data from two or more tables (e.g. profit = revenue - cost, attribution of costs to customers/products), check the "Table relationships" section of the schema ground truth: a declared join path must connect those tables. If NO path connects them, the data CANNOT be attributed at that granularity — do not invent a join, assume a bridge key, or relocate a column onto another table. Instead, narrow the plan to what IS computable from the declared paths (e.g. per-entity revenue from linked tables + aggregate-level totals from unlinked tables) and state the data-model limitation explicitly in the plan and in the Target architecture section. Delivering a smaller metric that is correct beats a richer metric the schema cannot support.
