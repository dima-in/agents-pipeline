...
- **Commands in** — the engine *asks* instead of listening: a long-poll `GET <…/commands>?project=&wait=`, opened only while a blocked run waits (`command_window_seconds`), so a green run never waits. Read-only `diff | diagnosis | cost | tasks` are answered as a `status` event carrying `reply_to: <command id>`, in markdown the chat renders — a fenced ```` ```diff ```` block, a task list, bold-labelled paragraphs — while every other event stays one line. Every command is stamped with the run it was issued for, and one from another `run_id` is refused with a threaded reply instead of silence: commands queue while nobody polls, and an older run's command must neither be applied to nor answered from a new one. `answer` (the owner's choice, as the option's text) is recorded; executing it is not automated yet.
